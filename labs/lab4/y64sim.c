...
//change
    bool_t ovf=FALSE;
    ovf= ((op==A_ADD) && (((argA>0) == (argB>0)) && ((argA>0) != (val>0))));
    ovf= ((op==A_SUB) && (((-argA>0) == (argB>0)) && ((argB>0) != (val>0))));
//change
...
//change
    if(icode!=I_ALU && icode!=I_JMP && icode!=I_RRMOVQ && icode!=I_DIRECTIVE && ifun!=0){
...
//change
    byte_t rAB;
    regid_t rA, rB;
    if(icode!=I_JMP && icode!= I_CALL && icode!=I_RET && icode!=I_NOP && icode !=I_HALT && icode!=I_DIRECTIVE){
        ...
        rB=GET_REGB(rAB);
//change
        if(icode!=I_IRMOVQ && rA==REG_NONE){
            err_print("PC = 0x%lx, Invalid instruction %.2x %.2x", sim->pc, codefun,rAB);
            return STAT_INS;
        }
       if(icode!=I_PUSHQ && icode!=I_POPQ && rB==REG_NONE){
            err_print("PC = 0x%lx, Invalid instruction %.2x %.2x", sim->pc, codefun, rAB);
            return STAT_INS;
        }
//change
...
//change
    long_t valC;
    if(icode==I_IRMOVQ || icode==I_RMMOVQ || icode==I_MRMOVQ || icode==I_JMP || icode==I_CALL || icode==I_DIRECTIVE){
    ...
    switch (icode) {
      ...
      case I_RRMOVQ: {
          ...
          if(cond_doit(sim->cc,cond)){
              ...
              alu_t op=A_NONE;
              long_t valE=compute_alu(op, valA, 0);
      ...
      case I_IRMOVQ: {
          alu_t op=A_NONE;
          long_t valE=compute_alu(op, valC, 0);
          set_reg_val(sim->r, rB, valE);
          sim->pc=next_pc;
          break;
      }/* 3:0 F:regB imm */
      case I_RMMOVQ: {
          ...
          alu_t op=A_NONE;
          long_t valE=compute_alu(op, valC, valB);
      ...
      case I_CALL: {
          ...
          alu_t op=A_NONE;
          long_t valE=compute_alu(op, -8, valB);
      ...
      case I_POPQ: {
          ...
          set_reg_val(sim->r, REG_RSP, valE);
          if(!get_long_val(sim->m, valA, &valM)){
              err_print("PC = 0x%lx, Invalid stack adress 0x%lx", sim->pc, valA);
              return STAT_ADR;
          }
          set_reg_val(sim->r, rA, valM);
          sim->pc=next_pc;
      ...
      case I_DIRECTIVE:{
          dtv_t dtv=ifun;
          if(dtv>D_ALIGN){
               err_print("PC = 0x%lx, Inalid instruction %.2x", sim->pc, codefun);
               return STAT_ADR;
          }
          if(dtv==D_DATA){
              sim->pc=next_pc+8*valC;
              break;
          }else if(dtv==D_POS){
              sim->pc=valC;
              break;
